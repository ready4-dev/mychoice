fit_choice_mdl <- function(dce_design_ls,
                           records_ls,
                           mdl_params_ls,
                           return_1L_chr = "mnl",
                           correlation_1L_lgl = F,
                           ...
                           ...){
  args_ls <- list(...)
  if(is.na(indl_predrs_chr[1]) & !identical(indl_predrs_chr, character(0))){
    indl_predrs_chr <- if(return_1L_chr %in% c("mnl","mixl","gmnl")){
      character(0)
    }else{
  ...
  f_fml <- make_choice_mdlng_fml(dce_design_ls$choice_sets_ls,
                                 candidate_predrs_chr = indl_predrs_chr,
                                 choice_var_nm_1L_chr = records_ls$choice_var_nm_1L_chr,
                                 ...
                                 use_mlogit_pkg_1L_lgl = use_mlogit_pkg_1L_lgl,
                                 wtp_mdl_1L_lgl = !is.null(smnl_mdl))
  random_params_xx <- if(return_1L_chr %in% c("mnl","lc","smnl")){NULL}else{mdl_params_ls$random_params_chr}
  if(return_1L_chr == "gmnl" & !is.null(smnl_mdl)){
    random_params_xx <- random_params_xx[names(random_params_xx)!=records_ls$cost_var_nm_1L_chr]
  }
  if(return_1L_chr == "smnl" | (return_1L_chr == "gmnl" & !is.null(smnl_mdl))){
    if(return_1L_chr == "smnl"){
      fixed_lgl <- c(make_choice_atts(dce_design_ls$choice_sets_ls,
                                      opt_out_var_nm_1L_chr = records_ls$opt_out_var_nm_1L_chr) %>%
                       purrr::map_lgl(~ifelse(.x %in% get_atts(dce_design_ls$choice_sets_ls$att_lvls_tb,"cont"),T,F)),c(T,F))
    ...
    if(return_1L_chr == "gmnl"){
      ...
      cost_idx_1L_int <- 1
      start_xx <- c(append(coefs_dbl,
                           c(1),
                           after = cost_idx_1L_int - 1),
                    rep(0.1,
                        ifelse(correlation_1L_lgl,
                               (.5 * length(random_params_xx) * (length(random_params_xx) + 1)),
                               length(coefs_dbl) -1)),
                    0.1,0)
    ...
    args_ls <- append(args_ls,
                      list(fixed = fixed_lgl,  start = start_xx))

  }
  if(((return_1L_chr %in% c("mm","smnl")) | (return_1L_chr == "gmnl" & !is.null(smnl_mdl))) & is.null(args_ls$iterlim)){
    args_ls$iterlim <- 500
  }
  panel_xx <- if(return_1L_chr == "mnl"){F}else{T}
  R_int <- if(return_1L_chr == "smnl"){1}else{mdl_params_ls$draws_1L_int}

  data_xx <- if(!return_1L_chr %in% c("lc","smnl","gmnl")|(return_1L_chr == "gmnl" & is.null(smnl_mdl))){ ##
    records_ls$ds_dfidx
  }else{
    if(return_1L_chr == c("lc")){
      make_choice_mdlng_ds(case_choices_mat = records_ls$case_choices_mat,
                           ...
                           return_1L_chr = "mlogit.data",
                           types_chr = character(0),
                           as_selection_ls_1L_lgl = F)
    }else{
      ds_dfidx <- records_ls$ds_dfidx
      ds_dfidx[,records_ls$cost_var_nm_1L_chr] <- ds_dfidx[,records_ls$cost_var_nm_1L_chr]*-1
      ds_dfidx
    }}
  args_ls <- append(list(formula = f_fml,
                         data = data_xx,
                         correlation = correlation_1L_lgl,
                         model = return_1L_chr,
                         haltons = halton_xx,
                         mvar = mvar_ls,
                         panel = panel_xx,
                         Q = nbr_of_clss_1L_int,
                         R = R_int,
                         ranp = random_params_xx),
                    args_ls)
  if(use_mlogit_pkg_1L_lgl){
    ...
    args_ls$mvar <- NULL
    args_ls$Q <- NULL
    model_mdl <- mlogit::mlogit(formula = f_fml,
                                data = args_ls$data,
                                correlation = args_ls$correlation,
                                halton = args_ls$halton,
                                panel = args_ls$panel,
                                R = args_ls$R,
                                rpar = args_ls$rpar,
                                ...)
    ...
    if(!is.null(args_ls$fixed)){
      ...
      model_mdl <- gmnl::gmnl(formula = f_fml,
                              data = args_ls$data,
                              correlation = args_ls$correlation,
                              model = args_ls$model,
                              haltons = args_ls$haltons,
                              mvar = args_ls$mvar,
                              panel = args_ls$panel,
                              Q= args_ls$Q,
                              R = args_ls$R,
  ...
  names_chr <- setdiff(intersect(names(model_mdl$call),names(args_ls)),"data")
  new_call_ls <- names_chr %>%
    purrr::map(~ if(is.character(args_ls[[.x]])){args_ls[[.x]]}else{paste0(deparse(args_ls[[.x]]), collapse = "") %>%
        stringr::str_squish()}) %>%
    stats::setNames(names_chr)
  new_call_ls <- append(new_call_ls, list(data = "ds_tb"), after = 1)
  names_chr <- names(new_call_ls)
  new_call_ls <- names(model_mdl$call) %>% purrr::map2(model_mdl$call,
                                                       ~{
                                                         if(.x %in% names_chr){
                                                           return_xx <- new_call_ls[[.x]]
                                                           if(is.character(return_xx) & length(return_xx)==1 & .x!="model")
                                                             return_xx <- return_xx %>% str2lang()
                                                         }else{
                                                           return_xx <-  if(.y %>% deparse() %>% purrr::pluck(1) %>% startsWith("args_ls")){
                                                             "NULL" %>% str2lang()
                                                           }else{
                                                             .y
                                                           }}
