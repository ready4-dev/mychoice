add_age_and_area_cmprsns <- function(prpn_cmprsns_ls = list(),
                                     ds_tb,
  ...
  if(identical(recode_ls,list())){
    recode_ls <- list(macro_nm_1L_chr = "AUS",
                      meso_nms_chr = c("New South Wales", "Victoria", "Queensland", "South Australia", "Western Australia", "Tasmania", "Northern Territory", "Australian Capital Territory"))
  }
  if(is.null(age_group_lup)){
    age_group_lup <- tibble::tibble(Var_1_xx = c(T,F, NA),
                                    Var_2_xx = c("TRUE","FALSE",NA_character_),
                                    Var_3_xx = c("A15","A20","25"),
                                    Value_xx = c("15-19","20-25","20-25"))
  }
  ...
  population_tb <- read.csv(path_1L_chr) %>%
    dplyr::filter(!!rlang::sym(period_var_nm_1L_chr) == period_1L_chr) %>%
    dplyr::arrange(!!rlang::sym(age_group_var_nms_chr[3]), !!rlang::sym(area_group_var_nms_chr[3])) %>%
    dplyr::mutate(!!rlang::sym(area_group_var_nms_chr[2]) := !!rlang::sym(area_group_var_nms_chr[3]))
  if(!is.null(recode_ls)){
    population_tb <- population_tb %>%
      dplyr::mutate(!!rlang::sym(area_group_var_nms_chr[2]) := !!rlang::sym(area_group_var_nms_chr[2]) %>%
                      purrr::map_chr(~ifelse(.x == recode_ls$macro_nm_1L_chr,
                                             .x,
                                             recode_ls$meso_nms_chr[as.numeric(.x)]
                      )))
  }
  if(!is.null(area_group_var_nms_chr)){
    ...
    area_cmprsn_tb <- make_cmprsn_tbl(ds_tb,
                                      popl_var_nm_1L_chr = popl_var_nm_1L_chr,
                                      sample_var_nm_1L_chr = sample_var_nm_1L_chr,
                                      expected_dbl = grouped_area_tb %>% dplyr::pull(!!rlang::sym(popl_var_nm_1L_chr)) %>% stats::setNames(grouped_area_tb %>% dplyr::pull(!!rlang::sym(area_group_var_nms_chr[2]))),
                                      grouping_var_nms_chr = area_group_var_nms_chr,
  ...
  if(!is.null(age_group_var_nms_chr)){
    ...
    if(!is.null(recode_ls)){
      grouped_age_tb <- grouped_age_tb %>% dplyr::filter(!!rlang::sym(area_group_var_nms_chr[2]) == recode_ls$macro_nm_1L_chr)
    }
    grouped_age_tb <- grouped_age_tb %>%
      dplyr::mutate(!!rlang::sym(age_group_var_nms_chr[2]) := !!rlang::sym(age_group_var_nms_chr[3]) %>%
                      purrr::map(~ready4::get_from_lup_obj(age_group_lup,
                                                           match_value_xx = .x,
                                                           match_var_nm_1L_chr = "Var_3_xx",
    ...
    age_cmprsn_tb <- make_cmprsn_tbl(ds_tb %>%
                                       dplyr::mutate(!!rlang::sym(age_group_var_nms_chr[1]) := !!rlang::sym(age_group_var_nms_chr[1]) %>%
                                                       purrr::map(~ready4::get_from_lup_obj(age_group_lup,
                                                                                            match_value_xx = .x,
                                                                                            match_var_nm_1L_chr = "Var_2_xx",
                                                                                            target_var_nm_1L_chr = "Value_xx")) %>%
                                                       unlist()),
                                     popl_var_nm_1L_chr = popl_var_nm_1L_chr,
                                     sample_var_nm_1L_chr = sample_var_nm_1L_chr,
                                     expected_dbl = grouped_age_tb %>% dplyr::pull(!!rlang::sym(popl_var_nm_1L_chr)) %>% stats::setNames(grouped_age_tb %>% dplyr::pull(!!rlang::sym(age_group_var_nms_chr[2]))),
                                     grouping_var_nms_chr = age_group_var_nms_chr,
                                     is_pc_1L_lgl = is_pc_1L_lgl)
...
add_attempt_dur_red_flag <- function(data_tb,
                                     attempt_dur_min_1L_dbl,
                                     attempt_dur_var_nm_1L_chr){
  data_tb <- data_tb %>%
    dplyr::mutate(red_flag_attempt_dur_lgl = !!rlang::sym(attempt_dur_var_nm_1L_chr) %>%
                    purrr::map_lgl(~ ifelse(as.numeric(.x) < attempt_dur_min_1L_dbl,T,F)))
  return(data_tb)
}
...
add_country_red_flag <- function(data_tb,
                                 lat_lng_var_nms_chr,
                                 valid_countries_chr){
  ...
  data_tb <- data_tb %>%
    dplyr::mutate_at(.vars = lat_lng_var_nms_chr,
                     .funs = as.numeric)
  sf::sf_use_s2(FALSE)
  back_tick_names <- names(data_tb)[!names(data_tb) == names(data_tb) %>% make.names()]
  sfRegion <- sf::st_as_sf(data_tb %>%
                             dplyr::filter(!is.na(!!rlang::sym(lat_lng_var_nms_chr[1]))) %>%
                             dplyr::filter(!is.na(!!rlang::sym(lat_lng_var_nms_chr[2]))), coords = lat_lng_var_nms_chr)
  ...
  sfIntersection <- sf::st_intersection(sfCountry,sf::`st_crs<-`(sfRegion,sf::st_crs(sfCountry))) %>%
    dplyr::select(admin,
                  names(data_tb)[!names(data_tb) %in% c(lat_lng_var_nms_chr, back_tick_names)])
  data_tb <- dplyr::full_join(data_tb %>%
                                dplyr::select(names(data_tb)[!names(data_tb) %in% lat_lng_var_nms_chr]),
                              sfIntersection)
  data_tb <- data_tb %>%
    dplyr::mutate(red_flag_country_lgl = purrr::map_lgl(admin, ~ ifelse(.x %in% valid_countries_chr, F,T)))
  return(data_tb)
}
add_cost_comparison <- function(analysis_ls,
                                choices_ls,
                                dce_design_ls,
                                mdls_ls,
                                mdl_params_ls,
  ...
  analysis_ls$new_data_ls$cost_comparison_ls$ensemble_ls <- purrr::map(cost_range_dbl,
                                                                       ~ {
                                                                         ...
                                                                         new_choices_ls[[altv_to_modify_1L_int]][which(names(new_choices_ls[[altv_to_modify_1L_int]]) == records_ls$cost_var_nm_1L_chr)] <- .x
                                                                         new_choices_ls
                                                                       })
  analysis_ls$new_data_ls$cost_comparison_ls$predd_shares_ls <- with_chr %>% purrr::map(~{
    with_1L_chr <- .x
...
add_cut_pnts_cmprsn <- function(prpn_cmprsns_ls = list(),
                                cmprsn_nm_1L_chr,
                                ds_tb,
...
add_duplicate_text_red_flag <- function(data_tb,
  ...
  data_tb <- purrr::reduce(1:length(unique_by_case_var_nms_chr),
                           .init = data_tb,
                           ~ .x %>%
                             dplyr::mutate(!!rlang::sym(paste0("red_flag_duplicate_txt_var",.y,"_lgl")):= duplicated(!!rlang::sym(unique_by_case_var_nms_chr[.y]))))
  return(data_tb)
}
add_email_red_flag <- function(data_tb,
                               email_var_nm_1L_chr,
                               email_max_cnstv_digits_1L_int){
  email_max_cnstv_digits_1L_int <- as.integer(email_max_cnstv_digits_1L_int)
  data_tb <- data_tb %>% dplyr::mutate(red_flag_email_lgl = purrr::map(!!rlang::sym(email_var_nm_1L_chr), ~ get_digits_from_text(.x)) %>%
                                         purrr::map( ~ nchar(.x) %>% max()) %>%
                                         purrr::map_lgl(~ifelse(.x>=email_max_cnstv_digits_1L_int,T,F)))
  return(data_tb)
}
add_flags <- function(records_ls,
                      age_var_nm_1L_chr = character(0),
                      attempt_dur_min_1L_dbl = numeric(0),
                      attempt_dur_var_nm_1L_chr = character(0),
                      date_stamp_format_1L_chr = "DMY",
                      date_stamp_var_nm_1L_chr = character(0),
                      dob_format_1L_chr = "DMY",
                      dob_var_nm_1L_chr = character(0),
                      email_max_cnstv_digits_1L_int = integer(0),
                      email_var_nm_1L_chr = character(0),
                      flags_max_1L_int = 0, #
                      flags_tot_var_nm_1L_chr = "red_flag_count_int",#
                      lat_lng_var_nms_chr = character(0),
                      qltv_ax_green_flag_1L_chr = "Green",
                      qltv_ax_red_flag_1L_chr = "Red",
                      qltv_ax_tb = NULL,
                      qltv_ax_var_nm_1L_chr = character(0),
                      unique_by_case_var_nms_chr = character(0),
                      valid_countries_chr = character(0)){
  records_ls$flags_ls <- list(
    # Variables for age inconsistency red flag
    age_var_nm_1L_chr = age_var_nm_1L_chr,
    date_stamp_format_1L_chr = date_stamp_format_1L_chr,
    date_stamp_var_nm_1L_chr = date_stamp_var_nm_1L_chr,
    dob_format_1L_chr = dob_format_1L_chr,
    dob_var_nm_1L_chr = dob_var_nm_1L_chr,
    # Variables for too short completion time red flag
    attempt_dur_min_1L_dbl = attempt_dur_min_1L_dbl, # Check if used?
    ...
    email_var_nm_1L_chr = email_var_nm_1L_chr,
    # Variables for ineligbile location red flag
    lat_lng_var_nms_chr = lat_lng_var_nms_chr,
    valid_countries_chr = valid_countries_chr,
    # Variables for duplicate responses red flag
    unique_by_case_var_nms_chr = unique_by_case_var_nms_chr,
    # Variables for qualitative assessment flags
    flags_max_1L_int = flags_max_1L_int,
    flags_tot_var_nm_1L_chr = flags_tot_var_nm_1L_chr,
    qltv_ax_green_flag_1L_chr = if(!is.null(qltv_ax_tb)){qltv_ax_green_flag_1L_chr}else{character(0)},
    qltv_ax_red_flag_1L_chr =  if(!is.null(qltv_ax_tb)){qltv_ax_red_flag_1L_chr}else{character(0)},
    qltv_ax_tb = qltv_ax_tb,
    qltv_ax_var_nm_1L_chr = qltv_ax_var_nm_1L_chr)
  return(records_ls)
}
add_new_choice_cmprsn <- function(analysis_ls,
                                  dce_design_ls,
                                  mdl_params_ls,
                                  new_choices_ls,
                                  records_ls,
  ...
  comparison_idx_1L_int <- ifelse(!"new_data_ls" %in% names(analysis_ls),
                                  1,
                                  length(names(analysis_ls$new_data_ls)[startsWith(names(analysis_ls$new_data_ls),"comparison_")])+1)
  comparison_nm_1L_chr <- paste0("comparison_",comparison_idx_1L_int,"_ls")
  ...
  analysis_ls$new_data_ls[[comparison_nm_1L_chr]]$predd_share_tb <- with_chr %>%
    purrr::map_dfc(~{
      with_1L_chr <- .x
      tibble::tibble(!!rlang::sym(paste0(with_1L_chr,"_prediction")) := predict_mkt_share(dce_design_ls = dce_design_ls,
                                                                                          mdls_ls = mdls_ls,
                                                                                          mdl_params_ls,
...
add_prpn_cmprsns <- function(comparison_tb,
                             ds_tb,
                             grouping_var_nms_chr,
                             is_pc_1L_lgl = T,
                             popl_var_nm_1L_chr = "Population (per-cent)",
                             tmp_var_nm_1L_chr = "cmprsn_test_ls"){
  comparison_tb <- comparison_tb %>%
    dplyr::mutate(!!rlang::sym(tmp_var_nm_1L_chr) := !!rlang::sym(grouping_var_nms_chr[2]) %>%
                    purrr::map2(comparison_tb[[popl_var_nm_1L_chr]],
                                ~{
                                  total_1L_int <- ds_tb %>% dplyr::pull(!!rlang::sym(grouping_var_nms_chr[1])) %>% table() %>% purrr::pluck(.x)
                                  if(is.null(total_1L_int)){
                                    total_1L_int <- 0
                                  }
                                  lessR::Prop_test(n_succ = total_1L_int,
                                                   n_fail = length(ds_tb %>% dplyr::pull(!!rlang::sym(grouping_var_nms_chr[1])) %>% na.omit()) -  total_1L_int,
                                                   pi = .y/ifelse(is_pc_1L_lgl, 100, 1))
                                })) %>%
    dplyr::mutate(CI_95_Lower = !!rlang::sym(tmp_var_nm_1L_chr) %>% purrr::map_dbl(~ifelse(!is.null(.x),.x$conf.int[1] * ifelse(is_pc_1L_lgl, 100, 1), NA_real_)),
                  CI_95_Upper = !!rlang::sym(tmp_var_nm_1L_chr) %>% purrr::map_dbl(~ifelse(!is.null(.x),.x$conf.int[2] * ifelse(is_pc_1L_lgl, 100, 1),NA_real_)),
                  p = !!rlang::sym(tmp_var_nm_1L_chr) %>% purrr::map_dbl(~ifelse(!is.null(.x),.x$p.value,NA_real_)))
  comparision_tb <- comparison_tb %>%
    rstatix::add_significance(p.col = "p") %>%
    dplyr::select(-!!rlang::sym(tmp_var_nm_1L_chr))
  return(comparision_tb)
}
add_qltv_ax_red_flag <- function(data_tb,
                                 qltv_ax_red_flag_1L_chr,
                                 qltv_ax_var_nm_1L_chr){
  data_tb <- data_tb %>%
    dplyr::mutate(red_flag_qltv_ax_lgl = purrr::map_lgl(!!rlang::sym(qltv_ax_var_nm_1L_chr), ~ ifelse(.x==qltv_ax_red_flag_1L_chr,T,F)))
  return(data_tb)
}
add_red_flags <- function(data_tb, # Create class and method to implement this
                          age_var_nm_1L_chr = character(0),
                          attempt_dur_min_1L_dbl = numeric(0),
                          attempt_dur_var_nm_1L_chr = character(0),
                          date_stamp_format_1L_chr = "DMY", #"%Y-%m-%d",
                          date_stamp_var_nm_1L_chr = character(0),
                          dob_format_1L_chr = "DMY",
                          dob_var_nm_1L_chr = character(0),
                          email_max_cnstv_digits_1L_int = integer(0),
  ...
  if(!identical(valid_countries_chr, character(0))){
    data_tb <- add_country_red_flag(data_tb = data_tb,
                                    lat_lng_var_nms_chr = lat_lng_var_nms_chr,
                                    valid_countries_chr = valid_countries_chr)
    flags_chr <- c(flags_chr,"red_flag_country_lgl")
  }
  if(!identical(dob_var_nm_1L_chr, character(0)) | !identical(age_var_nm_1L_chr, character(0))){
    data_tb <- data_tb %>%
      add_reported_age_red_flag(date_stamp_var_nm_1L_chr = date_stamp_var_nm_1L_chr,
                                dob_var_nm_1L_chr = dob_var_nm_1L_chr,
                                age_var_nm_1L_chr = age_var_nm_1L_chr)
  ...
  if(!identical(email_var_nm_1L_chr, character(0))){
    data_tb <- data_tb %>%
      add_email_red_flag(email_var_nm_1L_chr = email_var_nm_1L_chr,
                         email_max_cnstv_digits_1L_int = email_max_cnstv_digits_1L_int)
    flags_chr <- c(flags_chr,"red_flag_email_lgl")
  }
  if(!identical(unique_by_case_var_nms_chr, character(0))){
    data_tb <- data_tb %>%
      add_duplicate_text_red_flag(unique_by_case_var_nms_chr = unique_by_case_var_nms_chr)
    flags_chr <- c(flags_chr,
                   paste0("red_flag_duplicate_txt_var",1:length(unique_by_case_var_nms_chr),"_lgl")) # Check
  }
  if(!identical(qltv_ax_var_nm_1L_chr, character(0))){
    data_tb <- data_tb %>% dplyr::left_join(qltv_ax_tb)
    data_tb <- data_tb %>%
      add_qltv_ax_red_flag(qltv_ax_red_flag_1L_chr = qltv_ax_red_flag_1L_chr,
                           qltv_ax_var_nm_1L_chr = qltv_ax_var_nm_1L_chr)
    flags_chr <- c(flags_chr,"red_flag_qltv_ax_lgl")
  }
  if(!identical(attempt_dur_min_1L_dbl, numeric(0))){
    data_tb <- data_tb %>%
      add_attempt_dur_red_flag(attempt_dur_min_1L_dbl = attempt_dur_min_1L_dbl,
                               attempt_dur_var_nm_1L_chr = attempt_dur_var_nm_1L_chr)
    flags_chr <- c(flags_chr,"red_flag_attempt_dur_lgl")
  }
  data_tb <- add_red_flag_totals(data_tb = data_tb,
                                 flags_chr = flags_chr,
                                 flags_tot_var_nm_1L_chr = flags_tot_var_nm_1L_chr)
  return(data_tb)
}
add_red_flag_totals <- function(data_tb,
                                flags_chr,
                                flags_tot_var_nm_1L_chr){
  flags_tb <- data_tb %>% dplyr::select(flags_chr)
  flags_tb[is.na( flags_tb)] <- FALSE
  data_tb <- data_tb %>%
    dplyr::mutate(!!rlang::sym(flags_tot_var_nm_1L_chr) := purrr::reduce(flags_tb,
                                                                         .init = 0,
                                                                         ~ .x + .y))
  return(data_tb)
}
add_reported_age_red_flag <- function(data_tb, # NEED TO VERIFY THAT THIS WORKS ON BOTH MAC AND PC
                                      date_stamp_var_nm_1L_chr,
                                      dob_var_nm_1L_chr,
                                      age_var_nm_1L_chr,
                                      date_stamp_format_1L_chr = "DMY", #"%Y-%m-%d",
                                      dob_format_1L_chr = "DMY"# "%Y-%m-%d %H:%M:%S"
){
  ...
  fmt_seq_ls <- list(date_stamp_format_1L_chr %>% tolower() %>% stringr::str_extract_all("y|m|d") %>% purrr::pluck(1) %>% unique(),
                     dob_format_1L_chr %>% tolower() %>% stringr::str_extract_all("y|m|d") %>% purrr::pluck(1) %>% unique())
  data_tb <-  data_tb %>%
    dplyr::mutate(der_date_of_input = !!rlang::sym(date_stamp_var_nm_1L_chr) %>%
                    strsplit("/") %>% # Need to generalise
                    purrr::map(~ISOdate(gsub( " .*$", "", .x[which(fmt_seq_ls[[1]]=="y")]),
                                        gsub( " .*$", "", .x[which(fmt_seq_ls[[1]]=="m")]),
                                        gsub( " .*$", "", .x[which(fmt_seq_ls[[1]]=="d")]))  %>%
                                 format(format = "%Y-%m-%d")) %>% unlist() %>% as.Date()
    ) #!!rlang::sym(date_stamp_var_nm_1L_chr) %>% as.numeric() %>% janitor::excel_numeric_to_date()
  data_tb <-  data_tb %>%
    dplyr::mutate(der_date_of_birth = !!rlang::sym(dob_var_nm_1L_chr) %>% #as.numeric() %>% janitor::excel_numeric_to_date()
                    strsplit("/") %>%
                    purrr::map(~ISOdate(gsub( " .*$", "", .x[which(fmt_seq_ls[[2]]=="y")]),
                                        gsub( " .*$", "", .x[which(fmt_seq_ls[[2]]=="m")]),
                                        gsub( " .*$", "", .x[which(fmt_seq_ls[[2]]=="d")]))  %>%
                                 format(format = "%Y-%m-%d")) %>% unlist() %>% as.Date()) %>%
    dplyr::mutate(der_age_from_dob = lubridate::interval(der_date_of_birth,
                                                         der_date_of_input) %/%
                    lubridate::years(1))
  data_tb <- data_tb %>%
    dplyr::mutate(red_flag_reported_age_lgl = purrr::map2_lgl(!!rlang::sym(age_var_nm_1L_chr),
                                                              der_age_from_dob,
                                                              ~ ifelse(.x==.y,F,T)))
  return(data_tb)
}
add_sias_totals <- function(data_tb,
                            itm_prefix_1L_chr,
  ...
  data_tb <-  data_tb %>%
    dplyr::mutate_at(dplyr::vars(dplyr::setdiff(dplyr::starts_with(itm_prefix_1L_chr),
                                                dplyr::matches('_5$|_9$|_11$'))),
                     .funs = list(der = ~ purrr::map_dbl(.,~ switch(as.numeric(.x),0,1,2,3,4) ))) %>%
    dplyr::mutate_at(dplyr::vars(dplyr::intersect(dplyr::starts_with(itm_prefix_1L_chr),
                                                  dplyr::matches('_5$|_9$|_11$'))),
                     .funs = list(der = ~ purrr::map_dbl(.,~ switch(as.numeric(.x),4,3,2,1,0) ))) %>%
    dplyr::rename_at(dplyr::vars(dplyr::contains(der_suffix)), list( ~paste(drvd_var_prefix_1L_chr,
                                                                            gsub(der_suffix, "", .),
                                                                            sep = "_")))
  data_tb <- data_tb %>%
    ...
    dplyr::mutate(!!rlang::sym(paste0(drvd_var_prefix_1L_chr,"_",sias_ctg_sfx_1L_chr)) := !!rlang::sym(paste0(drvd_var_prefix_1L_chr,"_",sias_ttl_sfx_1L_chr)) %>%
                    ...
                    factor(levels = c("Normal_Range", "Social_Phobia",  "Social_Anxiety"), ordered = T)) %>%
    return(data_tb)
}
